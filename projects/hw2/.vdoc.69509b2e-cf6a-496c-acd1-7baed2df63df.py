...
# log-likelihood for vector Y
def loglik_poisson(lam, Y):
    return np.sum(-lam + Y * np.log(lam) - gammaln(Y + 1))

# plot log-lik over a grid
lams = np.linspace(0.1,  np.mean(Y)*3, 200)
lls  = [loglik_poisson(l, Y) for l in lams]

plt.plot(lams, lls)
plt.axvline(Y.mean(), color="red", linestyle="--",
            label=f"Sample mean = {Y.mean():.2f}")
plt.xlabel("λ")
plt.ylabel("Log‐Likelihood")
...
# numeric MLE
res = minimize_scalar(lambda lam: -loglik_poisson(lam, Y),
                      bounds=(1e-6, Y.mean()*3), method="bounded")
print(f"Numeric MLE: λ = {res.x:.4f}, sample mean = {Y.mean():.4f}")
#
#
#
#
...
#
#
# 1. Check which dtypes are object
print(X_df.dtypes)

# 2. Coerce everything to float
X_df = X_df.astype(float)

# (Also ensure y is numeric)
y = df["patents"].astype(float)

# 3. Now fit the Poisson GLM
...
#
#
# prepare X0 (all iscustomer=0) and X1 (all iscustomer=1)
X0 = X_df.copy()
X1 = X_df.copy()
X0["iscustomer"] = 0
X1["iscustomer"] = 1

mu0 = np.exp((X0 @ results.params).values)
mu1 = np.exp((X1 @ results.params).values)

delta = mu1 - mu0
print("Average effect (Δ patents):", round(delta.mean(), 3))
